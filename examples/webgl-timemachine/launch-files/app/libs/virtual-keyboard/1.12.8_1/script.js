...
function virtualKeyboardChromeExtension_fireEvent(eventType, key, keyCode) {

  // https://bugs.chromium.org/p/chromium/issues/detail?id=327853
  // TLDR: keyCode can only be set when the KeyboardEvent is created from a WebKeyboardEvent, and you can't call that constructor from javascript
  // Workaround: Use generic events

  // Note: keypress event returns different keycodes for upper and lower case letters, unlike keydown/keyup which has the same keycode regardless of case

  // Note 2: Chrome does not preserve custom properties when you initiate an event from a Content script to the page (and vice versa), so we must inject a script in the page to take over this job.
  // Supposedly chrome.debugger is another way to trigger keyboard events but for the life of me I cannot get that to work.
  ...
  keyboardEvent.shiftKey = virtualKeyboardChromeExtensionShift;
...
function virtualKeyboardChromeExtension_click(key, skip) {
	if (top != self) {
		...
		switch (key) {
			...
			case 'Enter':
				if (virtualKeyboardChromeExtensionClickedElem != null) {
					if (virtualKeyboardChromeExtensionElemType == "textarea") {
						...
						delete form;
						//virtualKeyboardChromeExtensionClickedElem.dispatchEvent(virtualKeyboardChromeExtension_fireEvent("keydown", "Enter", 13));
            triggerKeyEvent(virtualKeyboardChromeExtensionClickedElem, 13, virtualKeyboardChromeExtensionShift);
						virtualKeyboardChromeExtensionState = false;
						if (virtualKeyboardChromeExtensionFullScreenState) {
							if (hardwareAcceleration) {
								document.getElementById('virtualKeyboardChromeExtension').style.setProperty("-webkit-transform", "translate3d(0,450px,0)");
            ...
            virtualKeyboardChromeExtensionClickedElem.dispatchEvent(virtualKeyboardChromeExtension_fireEvent("keydown", key, key.toUpperCase().charCodeAt(0)));
						var pos = virtualKeyboardChromeExtensionClickedElem.selectionStart;
						var posEnd = virtualKeyboardChromeExtensionClickedElem.selectionEnd;
						...
						virtualKeyboardChromeExtensionClickedElem.dispatchEvent(virtualKeyboardChromeExtension_fireEvent("keypress", key, key.charCodeAt(0)));
						virtualKeyboardChromeExtensionClickedElem.dispatchEvent(virtualKeyboardChromeExtension_fireEvent("keyup", key, key.toUpperCase().charCodeAt(0)));
						virtualKeyboardChromeExtension_dispatch_event();
					}
				}
				break;
...
function triggerKeyEvent(element, charCode, shiftEnabled) {
  // We cannot pass object references, so generate a unique selector
  var attribute = 'robw_' + Date.now();
  element.setAttribute(attribute, '');
  var selector = element.tagName + '[' + attribute + ']';

  var s = document.createElement('script');
  s.textContent = '(' + function(charCode, attribute, selector, shiftEnabled) {
      ...
      var getterShift = {get: function() {return shiftEnabled}};
      Object.defineProperties(event, {
          ...
          char: getterChar,
          shiftKey: getterShift
      });

      element.dispatchEvent(event);
  } + ')(' + charCode + ', "' + attribute + '", "' + selector + '", ' + shiftEnabled + ')';
